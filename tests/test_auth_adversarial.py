...
def db_path(tmp_path):
    ...
    import cortex.auth as _auth_mod
    _auth_mod._jwt_secret = None  # Reset cached secret for test isolation
...
class TestAutoGeneratedSecret:
    """When CORTEX_JWT_SECRET is not set, the secret is auto-generated and
    persisted in the database.  These tests verify the mechanism."""

    def test_secret_is_not_empty(self, db_path):
        secret = get_jwt_secret()
        assert secret, "JWT secret must never be empty"

    def test_secret_is_not_a_well_known_default(self, db_path):
        """The auto-generated secret must not be a predictable string."""
        secret = get_jwt_secret()
        well_known = {"atlas-cortex-change-me", "atlas-cortex-secret", "changeme", "secret"}
        assert secret not in well_known, "Secret must not be a well-known default"

    def test_secret_is_long_enough(self, db_path):
        """Auto-generated secret should be at least 64 bytes of entropy."""
        secret = get_jwt_secret()
        assert len(secret) >= 64, f"Secret too short: {len(secret)} chars"

    def test_forged_token_with_wrong_secret_rejected(self, db_path):
        """An attacker who guesses a random secret cannot forge tokens."""
        forged = jwt.encode(
...
class TestJWTRoundTrip:
    ...
    def test_sub_is_string(self, db_path):
    ...
    def test_expiry_is_in_future(self, db_path):
        token = create_token(1, "admin")
        p = decode_token(token)
        assert p["exp"] > time.time()

    def test_expiry_matches_config(self, db_path):
...
class TestTamperedToken:
    def test_modified_payload_rejected(self, db_path):
    ...
    def test_wrong_algorithm_rejected(self, db_path):
        ...
        token = jwt.encode(payload, get_jwt_secret(), algorithm="HS384")
        with pytest.raises(HTTPException):
            decode_token(token)

    def test_none_algorithm_rejected(self, db_path):
...
class TestMissingClaims:
    def test_token_without_sub(self, db_path):
        """Token that lacks 'sub' must be rejected (claim enforcement)."""
        payload = {"username": "admin", "exp": int(time.time()) + 3600}
        token = jwt.encode(payload, get_jwt_secret(), algorithm=JWT_ALGORITHM)
        with pytest.raises(HTTPException) as exc:
            decode_token(token)
        assert exc.value.status_code == 401
        assert "missing required claims" in exc.value.detail.lower()

    def test_token_without_username(self, db_path):
        """Token that lacks 'username' must be rejected."""
    ...
    def test_completely_empty_payload(self, db_path):
        """Token with no sub/username must be rejected."""
...
class TestGarbageTokens:
    ...
    def test_random_bytes(self, db_path):
        with pytest.raises(HTTPException):
            decode_token("not-a-jwt-at-all")

    def test_three_dots_no_content(self, db_path):
        with pytest.raises(HTTPException):
            decode_token("..")

    def test_unicode_garbage(self, db_path):
        with pytest.raises(HTTPException):
...
class TestPasswordHashing:
    ...
    def test_very_long_password_handled(self):
        """Passwords > 72 bytes are pre-hashed with SHA-256, so bcrypt never
        sees more than 64 bytes.  Hash and verify must work correctly."""
        long_pw = "A" * 100
        h = hash_password(long_pw)
        assert verify_password(long_pw, h)
        # A shorter password must NOT match the long one
        assert not verify_password("A" * 72, h)

    def test_exactly_72_bytes_no_prehash(self):
        """72-byte passwords should work directly without pre-hashing."""
        pw = "A" * 72
        h = hash_password(pw)
        assert verify_password(pw, h)

    def test_73_bytes_prehashed(self):
        """73-byte password triggers pre-hash; must still round-trip."""
        pw = "B" * 73
        h = hash_password(pw)
        assert verify_password(pw, h)
...
class TestRequireAdmin:
    ...
    async def test_token_with_different_secret(self, db_path):
